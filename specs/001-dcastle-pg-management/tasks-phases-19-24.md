# Tasks: Phases 19â€“24 (Billing and Owner Management)

**Scope**: User Stories 14â€“21 (Building/Room Management, Rate History, Billing, Employee Management, Dashboards)
...
```
Phase 19 (US14) â”€â”€â”
                  â”œâ”€> Phase 20 (US15, US16) â”€â”€â”
                  â”‚                           â”œâ”€> Phase 22 (US18) â”€â”€â”
Phase 21 (US17) â”€â”€â”˜                           â”‚                     â”œâ”€> Phase 24 (US20, US21)
                                              â”‚                     â”‚
Phase 23 (US19) â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€â”˜
```

- **Phase 19** (Building/Room/Cot) â†’ Foundational; no external dependencies
- **Phase 20** (Rate History) â†’ Depends on Phase 19 (room structure)
- **Phase 21** (Mess Facility) â†’ Can run in parallel with Phase 19/20
- **Phase 22** (Billing) â†’ Depends on Phase 20 (rate lookups)
- **Phase 23** (Employee) â†’ Independent; no hard dependencies
- **Phase 24** (Dashboards) â†’ Depends on Phases 20, 22, 23
- **Phase 26** (Accommodation Assignment/Reassignment) â†’ Depends on Phase 19 (building/room/cot infrastructure)
...
### T100 â€” Phase 19: Database Migrations for Building/Room/Cot Tables

**User Story**: US14  
**Functional Requirements**: FR-084, FR-085, FR-086, FR-087  
**Acceptance Criteria**:
- [x] AC1: Migration creates `buildings` table with columns: `id`, `owner_id` (FK), `name`, `description`, `created_at`, `updated_at`, and unique constraint on `(owner_id, name)`
...
- [x] AC5: All tables include appropriate indexes on foreign keys and frequently-queried columns
- [x] AC6: Migration is idempotent and can be run against any database state
...
### T101 â€” Phase 19: API Routes for Building Management (CRUD)

**User Story**: US14  
**Functional Requirements**: FR-085 (Building CRUD)  
**Acceptance Criteria**:
- [x] AC1: `POST /api/admin/buildings` creates a new building; enforces `(owner_id, name)` uniqueness; returns `{ id, name, description, created_at }`
- [x] AC2: `GET /api/admin/buildings` returns all buildings for the authenticated owner with hierarchical rooms and cots nested
- [x] AC3: `GET /api/admin/buildings/[id]` returns a single building with full hierarchy (rooms â†’ cots) or 404 if not found
...
- [x] AC5: `DELETE /api/admin/buildings/[id]` soft-deletes or returns 400 if building contains active rooms; RLS prevents cross-owner access
- [x] AC6: All routes include proper error handling (400 for validation, 401 for auth, 403 for RLS, 500 for server)
- [x] AC7: Responses follow consistent JSON structure with metadata (created_at, updated_at)

**Test Coverage Required**:
- Unit tests: Input validation (building name length, description), uniqueness checks
- API integration tests: Create â†’ read â†’ update â†’ delete workflow per owner; RLS isolation between owners
...
### T102 â€” Phase 19: API Routes for Room and Room-Type Management

**User Story**: US14  
**Functional Requirements**: FR-085, FR-086 (Room and RoomType CRUD)  
**Acceptance Criteria**:
- [x] AC1: `POST /api/admin/room-types` creates a new room type for the owner; enforces `(owner_id, name, sharing_capacity)` uniqueness; returns full record with `id`, `name` (AC/non-AC), `sharing_capacity`, `cot_count`
...
- [x] AC5: `PATCH /api/admin/rooms/[id]` updates room details (number, floor, rent); validates uniqueness after update
- [x] AC6: `DELETE /api/admin/rooms/[id]` removes room only if no cots are assigned to active hostelers; otherwise returns 400
- [x] AC7: RLS prevents cross-owner room access (owner A cannot modify owner B's rooms)
- [x] AC8: `POST /api/admin/rooms/[id]/configuration-change` accepts JSON body: `{ new_sharing_capacity: int, new_room_class: 'ac'|'non_ac', new_rent: decimal, effective_date: date }`
- [x] AC9: Validates: new_sharing_capacity >= 1, new_rent > 0, effective_date must be today or future (IST calendar date); returns HTTP 400 with specific error "Effective date cannot be in the past" if effective_date < today
- [x] AC10: Creates immutable `room_configuration_history` record (INSERT-only; no UPDATE/DELETE allowed after creation); room's active configuration changes only when effective_date is reached
- [x] AC11: `GET /api/admin/rooms/[id]` returns extended response including: `{ ...current fields..., pending_change: { new_sharing_capacity, new_room_class, new_rent, effective_date } | null }`
- [x] AC12: `GET /api/billing/room-config?room_id=[id]&date=[date]` returns `{ sharing_capacity, room_class, rent, effective_date }` for the room configuration effective on that specific date; lookup query pattern: `SELECT new_sharing_capacity, new_room_class, new_rent FROM room_configuration_history WHERE room_id=$id AND effective_date <= $date ORDER BY effective_date DESC LIMIT 1`
...
- Integration/component validation: Cross-story billing validation (create room with type change â†’ generate bill for date before/after effective_date â†’ verify billing uses correct type)
...
### T103 â€” Phase 19: API Routes for Cot Management and Hosteler Assignment

**User Story**: US14  
**Functional Requirements**: FR-087 (Cot CRUD and hosteler assignment)  
**Acceptance Criteria**:
- [x] AC1: `POST /api/admin/rooms/[id]/cots` creates cots within a room based on room-type cot_count; enforces `(room_id, cot_id_label)` uniqueness
...
- [x] AC4: `GET /api/admin/cots/availability` returns full hierarchical view: all buildings â†’ rooms â†’ cots with occupancy details (for cot dashboard)
- [x] AC5: When a hosteler is assigned during registration (`POST /api/admin/hostelers`), cot assignment is atomic (building_id, room_id, cot_id all set together)
- [x] AC6: When a hosteler is deleted or deactivated, their cot is automatically unassigned (hosteler_id set to null)
...
- Integration/component validation: Assign hosteler to building â†’ room â†’ cot; verify hierarchical response; deactivate hosteler and verify cot freed
...
### T104 â€” Phase 19: Owner UI for Building/Room/Cot Management

**User Story**: US14  
**Functional Requirements**: FR-085, FR-086, FR-087 (UI implementation)  
**Acceptance Criteria**:
- [x] AC1: Owner dashboard includes "Buildings & Rooms" page at `/admin/buildings`
- [x] AC2: Page displays hierarchical tree: Buildings â†’ Rooms â†’ Cots with visual collapse/expand
- [x] AC3: Owner can click "Add Building" â†’ form appears for building name and description â†’ submit creates building via `POST /api/admin/buildings`
- [x] AC4: Owner can click "Add Room" within building â†’ form appears for room number, floor, room type selection, rent â†’ submit creates room via `POST /api/admin/buildings/[id]/rooms`
- [x] AC5: Owner can click "Add Room Type" â†’ form appears for AC/non-AC dropdown and sharing capacity (no base rent field), plus cot count â†’ submit creates room type via `POST /api/admin/room-types`
- [x] AC6: Owner can click "Configure Cots" on room â†’ form/modal shows cot creation and assignment workflow â†’ cots are created via `POST /api/admin/rooms/[id]/cots`
- [x] AC7: Forms include validation (required fields, numeric rent, enum dropdowns for floor/cot type)
- [x] AC8: Success feedback (toast, snackbar) after create/update; error feedback with specific messages
- [x] AC9: On 375 px mobile baseline: no page-level horizontal overflow, all form fields are reachable, buttons are touch-friendly (â‰¥44 px)
- [x] AC10: Owner can click "Change Room Configuration" button on any room card â†’ modal/form appears with: (a) sharing capacity input (1/2/3/4...), (b) AC/non-AC selector, (c) rent input, (d) date picker for effective date (default to today)
- [x] AC11: Form submission calls `POST /api/admin/rooms/[id]/configuration-change` with `{ new_sharing_capacity, new_room_class, new_rent, effective_date }`; on success, room card displays label in warning color: "Room configuration will be updated on [effective_date]" until the effective date is reached; on error (effective_date in past), error message displays inline below date picker: "Effective date cannot be in the past"
- [x] AC12: Date picker validation: (a) disallows selection of dates in the past (< today, IST calendar date), (b) submit button is disabled if effective_date < today, (c) date picker UI indicates "today" as default/highlighted option
- [x] AC13: On 375 px mobile: date picker form is fully rendered in viewport without horizontal scroll; input fields and buttons are touch-friendly (â‰¥44 px tall); modal/form does not overlap with primary content
...
- Integration/component validation: Create room â†’ schedule configuration change (sharing + AC/non-AC + rent) for future date â†’ verify pending label â†’ advance calendar to effective date â†’ verify label removed and new configuration applied
...
### T105 â€” Phase 19: Integration of Cot Assignment in Hosteler Registration

**User Story**: US14  
**Functional Requirements**: FR-087 (hosteler assignment to cots during registration)  
**Acceptance Criteria**:
- [x] AC1: Owner registration form for new hosteler includes cascading dropdowns: Building â†’ Room â†’ Cot
...
- [x] AC5: Submission to `POST /api/admin/hostelers` includes building_id, room_id, cot_id in request body
- [x] AC6: On success, hosteler is created and cot is atomically assigned (hosteler_id updated)
- [x] AC7: Validation prevents submission if any required building/room/cot field is missing
- [x] AC8: On 375 px mobile: cascading dropdown UX is accessible and not horizontally scrollable
...
- Integration/component validation: Open hosteler registration â†’ select building â†’ room â†’ cot â†’ submit â†’ verify hosteler and cot assignment
...
- [x] AC5: Migration is idempotent (IF NOT EXISTS guards)
...
**Acceptance Criteria**:
- [x] AC1: Unit tests for effective-date validation: (a) reject dates in past, (b) accept today/future (IST calendar date), (c) edge cases: midnight IST boundary, end-of-month, leap years, month transitions
...
- [x] AC4: Cross-story integration validation (non-E2E required): (a) owner creates room with initial configuration (4-sharing non_ac), (b) owner schedules room configuration change to 2-sharing ac with future date, (c) generate bill for date before effective_date â†’ verify old configuration-based rent, (d) advance IST calendar date to effective_date, (e) generate bill for date on/after effective_date â†’ verify new configuration-based rent
- [x] AC5: Immutability proof test: attempt to PATCH/UPDATE historical room_configuration_history row â†’ verify HTTP 403 Forbidden or 400 Bad Request with appropriate error message
...
- Integration/component validation: Cross-story billing validation (room configuration change â†’ bill generation for multiple dates)
...
**Acceptance Criteria** (Phase-level):
- [ ] Owner can set future global room rent config by sharing capacity and room type
...
### T106 â€” Phase 20: Database Migrations for Rate History Tables

**User Story**: US15, US16  
**Functional Requirements**: FR-088, FR-089  
**Acceptance Criteria**:
- [ ] AC1: Migration creates `room_rent_config_history` table with columns: `id`, `owner_id` (FK), `sharing_capacity`, `room_class` (`ac`/`non_ac`), `old_rent`, `new_rent`, `effective_date`, `created_by` (FK to owner), `created_at`, and unique constraint on `(owner_id, sharing_capacity, room_class, effective_date)`
...
- [ ] AC3: Indexes created on `owner_id`, `sharing_capacity`, `room_class`, `effective_date`, `meal_type` for efficient historical lookups
...
### T107 â€” Phase 20: API Routes for Global Room Rent Config Management

**User Story**: US15  
**Functional Requirements**: FR-088 (global room rent config change with effective date)  
**Acceptance Criteria**:
- [ ] AC1: `POST /api/admin/room-rent-config/change` accepts body: `{ sharing_capacity: int, room_class: 'ac'|'non_ac', new_rent: decimal, effective_date: date }`
- [ ] AC2: Creates a `room_rent_config_history` record keyed by `(owner_id, sharing_capacity, room_class, effective_date)`
- [ ] AC3: Until effective date, settings display shows "Rent config will be updated to â‚¹[new_rent] on [date]" for the selected sharing-capacity + room-class combination
- [ ] AC4: On/after the effective date, billing lookup for that combination returns the new rent
- [ ] AC5: Validates: sharing_capacity >= 1, room_class enum valid, new_rent > 0, effective_date is not in past (can be today or future)
- [ ] AC6: Supports date range: previous month (e.g., -30 days), current month, future dates
- [ ] AC7: `GET /api/admin/room-rent-config` returns current rent config matrix and pending changes per combination
- [ ] AC8: RLS prevents cross-owner rent-config modifications
...
### T108 â€” Phase 20: API Routes for Meal Rate Change Management and Historical Lookups
...
- Integration/component validation: Set future meal rate â†’ generate bill for future date â†’ verify bill uses new rate
...
### T109 â€” Phase 20: Owner UI for Rate Change Management

**User Story**: US15, US16  
**Functional Requirements**: FR-088, FR-089 (UI for rate changes)  
**Acceptance Criteria**:
- [ ] AC1: Settings page at `/admin/settings` includes sections: "Global Room Rent Config" and "Meal Rate Changes"
- [ ] AC2: Owner can choose sharing capacity + room class in Room Rent Config, then set date and new rent
- [ ] AC3: Form submission calls `POST /api/admin/room-rent-config/change`; success shows "Rent config change scheduled"
- [ ] AC4: Settings page displays pending label per combination: "Rent config will be updated to â‚¹[new] on [date]"
- [ ] AC5: Owner can click "Update Meal Rates" â†’ modal appears with date picker and input fields for breakfast, lunch, dinner
...
- Integration/component validation: Set global room-rent config change (sharing capacity + room class) with future date â†’ verify label displays â†’ advance date to effective date â†’ verify billing lookup uses new rent
...
### T110 â€” Phase 20: Unit Tests and Query Validation for Historical Rate Lookups

**User Story**: US15, US16  
**Functional Requirements**: FR-088, FR-089 (testing historical lookups)  
**Acceptance Criteria**:
- [ ] AC1: Unit test suite covers global room-rent config lookup for dates: before first change, on change date, between changes, after latest change
...
- [ ] AC5: Tests confirm lookup returns correct rate when multiple config changes exist for a sharing-capacity + room-class combination and for meal types
...
**Acceptance Criteria**:
- [ ] AC1: Room rent and meal rate forms show pre-submit effective-date summary (current vs scheduled values)
- [ ] AC2: Pending-change chips remain visible and consistent after refresh
- [ ] AC3: Duplicate/invalid effective-date feedback appears inline and is actionable
- [ ] AC4: Tablet-primary layout remains scannable with no horizontal overflow
...
### T111 â€” Phase 21: Database Schema Update for Mess Facility Assignment
...
### T112 â€” Phase 21: Food Preference Defaults Based on Mess Facility Status
...
- Integration/component validation: Register hosteler NOT availing â†’ submit â†’ verify defaults are OFF; register hosteler availing â†’ submit â†’ verify defaults are ON
...
### T113 â€” Phase 21: Owner UI to Toggle Mess Facility Status
...
- Integration/component validation: Toggle mess status â†’ submit preferences â†’ verify new defaults are applied
...
**Goal**: Two-phase billing (generate â†’ review â†’ transmit) with per-day accurate rate lookups
...
### T114 â€” Phase 22: Database Migration for Monthly Bills Table
...
### T115 â€” Phase 22: Billing Calculation Logic (Room Rent + Meal Charges)
...
- [ ] AC9: Same-day reassignment rule: if a hosteler has multiple room assignments on the same IST date, billing uses only the latest assignment for that date (no intra-day/hourly proration)
- [ ] AC10: Uses owner-adjusted current-month food entries when present (latest persisted value for each affected day)
- [ ] AC11: Function is testable in isolation (no side effects)
...
- Integration tests: Seeded month with specific hostelers, rates, preferences â†’ generate bill â†’ verify totals are correct
...
### T116 â€” Phase 22: API Route for Bill Generation
...
- Integration/component validation: Generate bills â†’ verify they appear in generated state â†’ regenerate â†’ verify replaced
...
### T117 â€” Phase 22: API Route for Bill Transmission and Hosteler Bill Visibility
...
**Test Coverage Required**:
- API integration tests: Transmit bill â†’ verify status changes and transmitted_at is set
...
- Integration/component validation: Generate bill â†’ verify hosteler doesn't see it â†’ transmit â†’ verify hosteler can see it
...
### T118 â€” Phase 22: Owner UI for Bill Generation, Review, and Transmission
...
- Integration/component validation: Generate bills â†’ view detail â†’ transmit â†’ verify hosteler sees bill
...
### T119 â€” Phase 22: Hosteler UI for Bill Viewing
...
### T119a â€” Phase 22: Owner API and UI for Current-Month Food Entry Adjustments (Individual Hosteler)

**User Story**: US18  
**Functional Requirements**: FR-034a, FR-034b, FR-034c, FR-034d, FR-034e  
**Acceptance Criteria**:
- [ ] AC1: `PATCH /api/admin/food-preferences/adjust` accepts body: `{ hosteler_id, start_date, end_date, meals: { breakfast, lunch, dinner }, adjustment_reason }`
- [ ] AC2: Validates range belongs to current IST month only; rejects cross-month or future-month ranges with HTTP 400
- [ ] AC3: Requires non-empty `adjustment_reason`; persists audit fields per adjusted date: `adjusted_by_owner_id`, `adjusted_at`, `adjustment_reason`
- [ ] AC4: Allows adjustment even when affected `(hosteler_id, month)` already has `status='transmitted'` bill; adjusted values become hosteler-visible only after owner regenerates and retransmits the bill
- [ ] AC5: Updates only selected hosteler and selected date range; no side effects on other hostelers/dates
- [ ] AC6: Owner UI on hosteler detail/history screen allows selecting current-month start/end dates and meal toggles, shows confirmation summary before applying
- [ ] AC7: After successful apply, UI shows adjustment audit row count and reflects updated daily entries immediately in owner history table
- [ ] AC8: On 375 px mobile baseline: date range picker, meal toggles, reason textarea, and submit button are reachable without horizontal overflow
- [ ] AC9: If bill is already transmitted for that hosteler-month, UI shows guidance: "Changes saved. Regenerate and retransmit bill to publish updates to hosteler."

**Test Coverage Required**:
- API integration tests: Valid current-month adjustment succeeds; cross-month fails; missing reason fails; transmitted-bill month adjustment succeeds and flags regenerate/retransmit workflow
- Unit tests: Date-range validation in IST month boundary conditions
- Component tests: Owner adjustment form validation, confirmation, success/error states
- Integration/component validation: Owner adjusts hosteler entries in current month after transmission â†’ regenerates and retransmits bill â†’ verifies hosteler sees updated amounts

**Dependencies**: T115 (billing calculation logic), T116 (bill generation API), T118 (owner billing UI)

**File Paths**:
- Routes: `src/app/api/admin/food-preferences/adjust/route.ts`, `src/app/api/admin/food-preferences/adjustments/route.ts`
- Components: `src/components/hostelers/food-adjustment-form.tsx`
- Tests: `src/app/api/admin/food-preferences/__tests__/adjust.test.ts`, `src/components/hostelers/__tests__/food-adjustment-form.test.tsx`, `e2e/us18-owner-food-adjustments.spec.ts`
...
**Acceptance Criteria**:
- [ ] AC1: Owner bill list supports status filter, hosteler search, and sticky month/scope context
- [ ] AC2: Owner and hosteler bill details support collapsed/expanded breakdown mode
- [ ] AC3: High-volume bill list remains navigable without losing active filter context
...
### T120 â€” Phase 23: Database Migrations for Employee and Salary History Tables
...
### T121 â€” Phase 23: API Routes for Employee Management (CRUD)

**User Story**: US19  
**Functional Requirements**: FR-093 (employee CRUD)  
**Acceptance Criteria**:
- [ ] AC1: `POST /api/admin/employees` creates new employee; body: `{ name, job_description, salary }` â†’ returns `{ id, name, job_description, current_salary }`
...
### T122 â€” Phase 23: API Route for Salary Changes and Historical Lookups
...
- [ ] AC3: Until effective date, employee list shows "Salary will be updated to â‚¹[new] on [date]"
...
- [ ] AC7: Supports multiple salary changes per employee with correct precedence (latest effective date â‰¤ lookup date)
...
- Integration/component validation: Set future salary change â†’ verify label â†’ verify profit dashboard uses historical salary for past periods
...
### T123 â€” Phase 23: Owner UI for Employee Management
...
- [ ] AC3: "Add Employee" button opens form with fields: name, job_description, initial salary â†’ submission calls `POST /api/admin/employees`
- [ ] AC4: "Edit" opens form to update name/job_description â†’ calls `PATCH /api/admin/employees/[id]`
- [ ] AC5: "Update Salary" button opens modal with date picker and new salary input â†’ calls `POST /api/admin/employees/[id]/salary-change`
- [ ] AC6: Pending salary changes displayed as: "Salary will be updated to â‚¹[new] on [date]"
...
- Integration/component validation: Add employee â†’ set salary change â†’ verify label displays â†’ verify employee is queryable for profit dashboard
...
### T124 â€” Phase 24: Database Migration for Line-Item Expenses Table
...
### T125 â€” Phase 24: Profit Dashboard Calculation Logic (Income, Expenses, Profit)
...
- [ ] AC4: Profit = Income Total âˆ’ Expenses Total
...
- Integration tests: Seeded month with specific hostelers, rates, meals, employees â†’ calculate dashboard â†’ verify totals and breakdowns
...
### T126 â€” Phase 24: API Route for Profit Dashboard and Expense Management
...
- Unit tests: Month parameter parsing (handles any date â†’ first-of-month range)
...
### T127 â€” Phase 24: Owner UI for Profit Dashboard
...
- [ ] AC2: Dashboard displays three cards: Income, Expenses, Profit (with values and Â± comparison to previous month if applicable)
...
- [ ] AC6: "Add Expense" button opens modal/form with fields: description, amount, optional date-within-month â†’ submission calls `POST /api/admin/profit-dashboard/expenses`
...
- [ ] AC8: Breakdown detail includes dates/effective-date information (e.g., "Room rent changed from â‚¹[old] to â‚¹[new] on [date]")
...
- Integration/component validation: Load dashboard for specific month â†’ verify calculations â†’ add expense â†’ verify profit recalculates
...
### T128 â€” Phase 24: API Route for Cot Availability Dashboard

**User Story**: US21  
**Functional Requirements**: FR-096 (cot availability query)  
**Acceptance Criteria**:
- [ ] AC1: `GET /api/admin/cots/availability` returns hierarchical view: buildings â†’ rooms â†’ cots with occupancy status
...
- Integration tests: Assign hosteler to cot â†’ query â†’ verify occupancy updates; unassign â†’ verify cot is free again
- Integration/component validation: Navigate cot dashboard â†’ verify hierarchy and occupancy accuracy
...
### T129 â€” Phase 24: Owner UI for Available Cot Dashboard
...
- Integration/component validation: Assign hosteler â†’ refresh dashboard â†’ verify cot is marked occupied with hosteler name; deactivate hosteler â†’ verify cot freed
...
### T130 â€” Phase 24: Integration Validation for Profit Dashboard and Cot Occupancy
...
- [ ] AC2: Test steps:
  ...
  4. Verify profit = income âˆ’ expenses
  5. Add line-item expense â†’ verify profit recalculates immediately
  ...
  10. Deactivate a hosteler â†’ refresh cot dashboard â†’ verify their cots are now free
...
- [ ] AC2: Large breakdown sections are collapsed by default and expandable on demand
- [ ] AC3: State transitions (loading -> data, loading -> empty, loading -> error) are deterministic and test-covered
...
## Phase 26: Hosteler Accommodation Assignment and Reassignment (US22)
...
**Acceptance Criteria** (Phase-level):
- [ ] Owner can assign building -> room -> cot for an existing hosteler from a dedicated page
- [ ] Owner can reassign a hosteler to a different room/cot
- [ ] Reassignment atomically releases old cot and assigns new cot
- [ ] Occupied-cot assignment attempts are blocked with clear feedback
- [ ] Hosteler can remain unassigned until owner explicitly assigns accommodation
...
**Acceptance Criteria**:
- [ ] AC1: Add assign/reassign endpoint for existing hosteler with payload `{ hosteler_id, building_id, room_id, cot_id }`
- [ ] AC2: Reassignment flow atomically clears previous cot assignment and assigns target cot
- [ ] AC3: Assignment to occupied cot returns validation error
- [ ] AC4: Unassign endpoint supports returning hosteler to unassigned state

**Test Coverage Required**:
- [ ] API integration tests for assign, reassign, occupied-cot rejection, and unassign

**Dependencies**: Phase 19 APIs

**File Paths**:
- [ ] Routes: `src/app/api/admin/hostelers/[id]/accommodation/route.ts`
- [ ] Tests: `src/app/api/admin/hostelers/__tests__/accommodation-route.test.ts`
...
**Acceptance Criteria**:
- [ ] AC1: New owner page lists hostelers with assignment status (assigned/unassigned)
- [ ] AC2: Page provides cascading selectors for building -> room -> cot and assign/reassign action
- [ ] AC3: Reassignment confirmation explains old cot release and new cot assignment
- [ ] AC4: UI shows clear error when selected cot is occupied

**Test Coverage Required**:
- [ ] Component tests for selector cascade and assign/reassign flows

**Dependencies**: T131

**File Paths**:
- [ ] Page: `src/app/admin/hostelers/accommodation/page.tsx`
- [ ] Components: `src/components/hostelers/accommodation-assignment-panel.tsx`
- [ ] Tests: `src/components/hostelers/__tests__/accommodation-assignment-panel.test.tsx`
...
**Acceptance Criteria**:
- [ ] AC1: Add Hosteler form requires only identity fields and no accommodation fields
- [ ] AC2: Registration success no longer depends on cot availability
- [ ] AC3: Hosteler list clearly indicates unassigned state until accommodation assignment occurs

**Test Coverage Required**:
- [ ] Component/API tests for identity-only registration and unassigned-state rendering

**Dependencies**: T105k, T132

**File Paths**:
- [ ] Components: `src/app/admin/hostelers/page.tsx`, `src/components/hostelers/add-hosteler-form.tsx`
- [ ] Tests: `src/app/admin/hostelers/__tests__/page.test.tsx`
...
|-------|-------|-------|-----------------|
| 19 | US14 | T100â€“T105 (6 tasks) | 1 Small, 2 Small, 1 Medium, 1 Medium, 1 Small = Medium total |
| 20 | US15, US16 | T106â€“T110 (5 tasks) | 1 Small, 1 Small, 1 Medium, 1 Medium, 1 Small = Medium total |
| 21 | US17 | T111â€“T113 (3 tasks) | 1 Small, 1 Small, 1 Small = Small total |
| 22 | US18 | T114â€“T119a (7 tasks) | 1 Small, 1 Medium, 1 Medium, 1 Medium, 1 Medium, 1 Small, 1 Medium = Large total |
| 23 | US19 | T120â€“T123 (4 tasks) | 1 Small, 1 Small, 1 Small, 1 Small = Small total |
| 24 | US20, US21 | T124â€“T130 (7 tasks) | 1 Small, 1 Medium, 1 Medium, 1 Small, 1 Medium, 1 Medium, 1 Medium = Mediumâ€“Large total |

**Total**: 32 tasks across 6 phases (Phases 19â€“24)

### Dependency Chain

```
Phase 19 (US14) â€” Building/Room/Cot Infrastructure
â””â”€â”€ T100: Database migrations
    â””â”€â”€ T101: Building CRUD API
        â””â”€â”€ T102: Room/RoomType CRUD API
            â””â”€â”€ T103: Cot/Assignment API
                â””â”€â”€ T104: Owner UI (Building/Room/Cot)
                    â””â”€â”€ T105: Hosteler Registration Integration

Phase 20 (US15, US16) â€” Rate History Tracking (depends on Phase 19)
â””â”€â”€ T106: Rate History Database
    â””â”€â”€ T107: Room Rent Change API
    â””â”€â”€ T108: Meal Rate Change API
        â””â”€â”€ T109: Owner UI (Rate Changes)
        â””â”€â”€ T110: Unit Tests (Historical Lookups)

Phase 21 (US17) â€” Mess Facility (can run parallel to 19/20)
â””â”€â”€ T111: Database Schema (availing_mess column)
    â””â”€â”€ T112: Food Preference Default Logic
        â””â”€â”€ T113: Owner UI (Mess Facility Toggle)

Phase 22 (US18) â€” Billing (depends on Phase 20)
â””â”€â”€ T114: Bills Database
    â””â”€â”€ T115: Billing Calculation Logic
        â””â”€â”€ T116: Bill Generation API
            â””â”€â”€ T117: Bill Transmission API
                â””â”€â”€ T118: Owner Billing UI
                â””â”€â”€ T119: Hosteler Bill UI
                â””â”€â”€ T119a: Owner Food Entry Adjustments (Current Month)

Phase 23 (US19) â€” Employee Management (independent)
â””â”€â”€ T120: Employee Database
    â””â”€â”€ T121: Employee CRUD API
        â””â”€â”€ T122: Salary Change API
            â””â”€â”€ T123: Owner UI (Employee Management)

Phase 24 (US20, US21) â€” Dashboards (depends on 20, 22, 23)
â””â”€â”€ T124: Line-Item Expenses Database
    â””â”€â”€ T125: Profit Calculation Logic
        â””â”€â”€ T126: Profit Dashboard API
            â””â”€â”€ T127: Profit Dashboard UI
â””â”€â”€ T128: Cot Availability API
    â””â”€â”€ T129: Cot Availability UI
        â””â”€â”€ T130: Integration Validation (Both Dashboards)
```
...
**Assessment**: All 31 tasks (T100â€“T130) in Phases 19â€“24 are currently unimplemented. Codebase contains no building/room/cot infrastructure, rate history tracking, billing schema/logic, employee management, or profit/cot dashboard features.
...
**Next Steps**:
1. Run `/speckit.implement` to systematically complete Phases 19â€“24 starting with Phase 19 (Building/Room/Cot) which has no external dependencies
...
**All tasks from Phases 19â€“24 remain in their original locations (above) and are ready for implementation**. This phase serves as a consolidation and tracking checkpoint. No task modifications are required; proceed with implementation as specified.
