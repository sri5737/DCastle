...
3. Do not run `npm run build:cloudflare` as a routine local completion gate. Run it only when the user explicitly requests it or when diagnosing a reported pipeline/build failure.
4. If any required test fails, fix the code before marking the phase complete.
5. New features MUST include corresponding test coverage using the cheapest meaningful test type: unit tests for logic, API integration tests for backend/persistence behavior, and component tests for UI states.
...
4. Run browser E2E specs only when explicitly requested by the user.
...
Do not repeatedly run optional browser suites while a narrower failing test remains unresolved.
...
- Validate affected hosteler screens in Android Chrome mobile viewport and validate installed/standalone PWA behavior where the screen is used from the installed app.

Do not mark hosteler-facing tasks complete if the affected screen breaks at the 375 px Android baseline, even when automated tests or Cloudflare build pass.
...
Do not mark owner-facing tasks complete if the affected screen is unusable on tablets (768px+), even when automated tests or Cloudflare build pass. Mobile rendering issues are acceptable and do not block completion.
...
- Critical user journeys should be validated through API integration and component coverage by default; add optional browser tests only when explicitly requested.
- Required tests MUST pass before the task is marked complete.

Detailed edge cases SHOULD be covered by unit, API integration, or component tests instead of browser tests. If optional browser tests are skipped, document the chosen test type and rationale in the task/spec evidence.

## Optional Browser Test Guidance

Browser E2E is optional going forward and is not a required completion gate. When explicitly requested, browser tests should still validate exact business outcomes and avoid placeholder assertions.
...
- Tests updated using the required mix of unit, API integration, and component coverage
